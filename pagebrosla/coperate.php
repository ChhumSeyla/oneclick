<?php 
            ...
            if (isset($_GET['bid'] ) )  {
              $sql_doc = "SELECT * FROM tbl_product where category='".$cid."' and brand='".$_GET['bid']."' and status=1 ORDER BY No desc ";
            }
            else if(isset($_GET['cid'])){
              $sql_doc = "SELECT * FROM tbl_product where category='".$_GET['cid']."' and status=1 ORDER BY No desc ";
            }
            else{
              ...
              <h3 class="card-title tr" key="Find Product by Brand" style="color: white" >Find Product by Brand</h3><br>
               ...
               <div class="card-body pad">
                <div class="form-group row accent-orange">
                   <?php 
                   if (isset($_GET['cid'])) {
                     $sql_doc = "SELECT * FROM tbl_brand  inner join tbl_product on tbl_brand.ID=tbl_product.brand where category='".$_GET['cid']."' GROUP BY ID order by ID asc  ";
                     $doc = mysqli_query($con,$sql_doc);
                     while($row = mysqli_fetch_assoc($doc) ){
                      echo ' <div class="col-md-2" data-aos="fade-up">
                      <a href="index.php?page=coperate&cid='.$row['category'].'&bid='.$row['brand'].'" >
